...
def export_paid_to_csv(modeladmin, request, queryset):
    '''allows the admin to export orders as spreadsheets: paid and unpaid orders separately'''
    ...
    writer.writerow([field.verbose_name for field in fields]) # the headers
    for obj in queryset:
        if not getattr(obj, "paid"): #unpaid orders
            continue
        data_row = []
        for field in fields:
...
class ShippingAdressAdmin(admin.ModelAdmin):
    list_display = ('first_name','last_name','user', 'email', 'city','country','zip_code')
...
class OrderItemInline(admin.TabularInline):
    ...
    def get_readonly_fields(self, request, obj=None):
        if obj:
            return ['price', 'item', "quantity", "user"]
